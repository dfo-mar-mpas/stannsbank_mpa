...
#eEuate operational buffer for 2024 season  -------------
...
## load priority locations for camera drops
    ...
    ggplot()+
      geom_sf(data=perley_buffers%>%filter(name=="Louisbourg"),fill=NA,lty=2)+
      geom_sf(data=sab_zones,fill=NA,lwd=0.5)+
      geom_sf(data=banks)+
      geom_sf(data=cam_coords,aes(col=rationale))+
      theme_bw()+
      labs(col="")+
      theme(legend.position = "bottom")
