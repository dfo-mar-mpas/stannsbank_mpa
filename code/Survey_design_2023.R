...
## load the shapefile with the camera transects
    cam_trans <- read_sf("data/Shapefiles/RV_transects_insideSAB/RV_sets_2017_2022_transects_SAB.shp")%>%
                 st_transform(latlong)
    
    cam_coords <- read_sf("data/Shape")
    
    ggplot()+
      geom_sf(data=sab)+
      geom_sf(data=cam_trans)+
      theme_bw()
