...
batch_n <- ifelse(is.na(batch_n),1,batch_n)
...
BACI <- data.frame(sp=c(richness_BACI$sp,crab_BACI$sp_name),
                   dependent=as.integer(c(richness_BACI$Richness,crab_BACI$CPUE)),
...
for(s in unique(b_data$sp)){
  ...
  spresults$p <- lapply(1:nrow(spresults),function(row){
    ...
    p <- try({
      ...
      r1 <- rep(sample(BACI$rand1,spresults$sites[row],replace=TRUE),2)
      r2 <- rep(sample(BACI$rand2,spresults$sites[row],replace=TRUE),2)
      r1index <- r1 %>%
        lapply(function(x){
      ...
      multifactor <- rep(c(1,if_else(spresults$trend[row]=="Increase",
      ...
      randomeffectsformula <-  "(1|rand1)+(1|rand2)"
      simdatawithoutperiod <- lme4::glmer.nb(formula= as.formula(paste0("dependent ~",randomeffectsformula)),
                                             data = simdata)
      
      if ( isSingular(simdatawithoutperiod) ){
        randeffects <- simdatawithoutperiod |> 
          ranef() |> 
          as.data.frame() |> 
          group_by(grpvar) |> 
          reframe(meanval=mean(abs(condval)))
        
        randomeffectsformula <- paste0("(1|",randeffects$grpvar[randeffects$meanval==max(randeffects$meanval)],")"  )
      ...
      simdatawithperiod <- lme4::glmer.nb(formula= as.formula(paste0("dependent ~ PERIOD+",randomeffectsformula)),
                                          data = simdata)
